...
We define a \emph{history}\index{history} to be a sequence of calls and
returns of operations, corresponding to threads operating on the datatype.
...
the \emph{linearization point}\index{linearization point}.
...
\emph{linearization}\index{linearization}~\cite{herlihy-wing}.  It is an
...
Figure~\ref{fig:linearization} gives four timelines\index{timeline} depicting
histories.  In each timeline, time goes from left to right.  Each row is
labelled with the identity of a thread (e.g.~$t_0$).  Each horizontal line
represents the execution of an operation, with the endpoints of the line
representing the call of the operation and its return, respectively; the line
is labelled with the name of the operation, its parameters, and the value
returned, although we omit the latter for operations, like |enqueue|, that
return the trivial |Unit| type.
...
history\footnote{We write sequences within angle brackets, $\seq{\ldots}$.}
\[\mstyle
...
certainly expect them to take effect in that order.  As a result, the
|dequeue| returns~|5|.  The timeline is linearizable, as shown by the labelled
linearization points.  However, if the |dequeue| had returned |Some(4)|, the
history would not be linearizable; it would not correspond to how we expect a
queue to behave.
...
%
\begin{enumerate}
\item Thread $t_1$ calls |enqueue(5)| and returns, but the value~|5| stays in
  the |enqueueChan|;

\item Thread $t_2$ calls |dequeue()|, and the server sends it |None|.
\end{enumerate}
%
This history is not linearizable: because the |enqueue| returns before the
|dequeue| is called, it would necessarily correspond to the sequential history
\[\mstyle
\seq{ \sm{enqueue(5)},\, \sm{dequeue():None}},
\]
which is not valid.

Alternatively, using buffered channels could lead to the following incorrect
behaviour: 
%
\begin{enumerate}
...
A \emph{concurrent history}\index{concurrent history} of an object~$o$ records
the calls and returns of operations on~$o$.  It is a sequence of events of the
following forms:
%
\begin{itemize}
...
\end{itemize}
%
Here $i$ is a \emph{execution identity}\index{execution identity}, used to
identify a particular execution, and to link the $\call$ and
corresponding~$\return$.  For example, timeline~1 in
Figure~\ref{fig:linearization} represents the history
\[\mstyle
\langle
  \begin{align}
...
We say that a $\call$ event and a $\return$ event \emph{match}\index{matching
events} if they have the same execution identifier.  A concurrent history is
\emph{complete}\index{complete history} if for every $\call$ event, there is a
matching $\return$ 
...
object\index{sequential specification object}~$Spec$, with the same operations
and signatures as the concurrent object in question.  For the case of a total
queue, we could define the sequential specification object as follows:
%
\begin{scala}
class SeqQueue[T]{
...
\end{scala}
%
A history\index{sequential history} of the specification object is a sequence
...
\end{itemize}
%
Such histories are designated as \emph{legal}\index{legal sequential history}
if they are consistent with the definition of~$Spec$.  Earlier, we gave
several legal histories for |SeqQueue| (although we omitted execution
identities).

Let $h$ be a complete concurrent history, and let $h_s$ be a legal history of
the specification object.  We say that $h$ and~$h_s$
\emph{correspond}\index{corresponding histories} if they contain the same
executions of operations, i.e., for each $\call.op^i(x)$ and
$\return.op^i\::y$ in $h$,\, $h_s$ contains $op^i(x)\::y$, and vice versa;
note that the execution identities indicate which events of the two histories
correspond to each other.  

We say that $h_s$ is a \emph{linearization}\index{linearization} of~$h$ if
there is some way of interleaving the two histories (i.e.,~creating a history
containing the events of~$h$ and~$h_s$, preserving the order of events) such
that each $op^i(x)\::y$ occurs between $\call.op^i(x)$ and $\return.op^i\::y$.
Informally, this indicates that the executions of~$h$ appeared to take place
in the order described by~$h_s$, and that this order is legal according to the
...
executions that have been called but have not returned.  An
\emph{extension}\index{extension} of a concurrent history~$h$ is formed by
adding zero or more $\return$ events corresponding to pending executions.  We
write $complete(h)$ for the subsequence of~$h$ formed by removing all $\call$
events corresponding to pending executions.
...
We say that a concurrent object is \emph{linearizable}\index{linearizable} if
all of its histories are linearizable.
