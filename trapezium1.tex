...
The trapezium rule\index{trapezium rule} is a standard numerical technique to
approximate the value of an integral.  It splits the integral's range $[a,b]$
into $n \ge 1$ intervals:
\[\mstyle
\begin{align}
...
Double} to {\scalashape Double}; see Section~\ref{sec:scala-types}.}.  (We
will see several implementations of the trapezium rule, so we factor out some
common code into an abstract class |TrapeziumT|; see
Section~\ref{sec:scala-encapsulation}.)
...
allows us to write |o(args)| as a shorthand for |o.apply(args)|; see
Section~\ref{sec:scala-misc}.  This is conventionally used for function-like
objects, so that the call of |apply| looks like a function application.  We
follow this convention here and later in the book.
...
of \emph{worker threads}\index{worker thread}.  We then split the interval
$[a,b]$ into the same number of equal-size subranges, so each worker works on
one subrange.  A \emph{controller}\index{controller} thread tells each worker
which subrange to work on.  Each worker sends its subresult back to the
controller, which adds them up to give the overall result.

This is a form of \emph{data parallelism}\index{data parallel}.  We split
the data (the value of~$f$ over the relevant range) between the workers.  Each
worker calculates a subresult on its part of the data, and then we combine the
subresults.
...
subresults from all the workers.  Hence, the in-port of |toWorkers| is
shared\index{channel!shared} between the workers, as is the out-port of
|toController|.

Figure~\ref{fig:trapezium-1} gives some of the code.  The companion object
|Trapezium| (see Section~\ref{sec:scala-encapsulation}) factors out some
definitions that we will reuse in a later implementation.  (We take the
parameter |n| of |Trapezium|, the number of intervals, to be a |Long|---see
Section~\ref{sec:scala-core}---because we want to experiment with large
...
\sm{r} - \sm{l}$).  We can pass this information as a 4-tuple \SCALA{(l, r,
  taskSize, delta)}.  We define a type |Task|\index{task} (see
Section~\ref{sec:scala-types}) to represent such 4-tuples, and create a
channel |toWorkers| to pass |Task|s.

Each worker returns a \SCALA{Double} to the controller, so we create a channel
|toController| to pass such values.

Note that the channels are \emph{private} (see
Section~\ref{sec:scala-encapsulation}): we want to ensure that only the
threads we define in the |Trapezium| class can send and receive on them.
...
|Int|s.  However, |Task|s (like all tuples) are reference objects (see
Section~\ref{sec:scala-types}).  When a reference object is sent on a channel,
...
builds a sequence of workers using a |for| expression (see
Section~\ref{sec:scala-misc}).  It then combines them together in parallel
using an indexed form of parallel composition: if |ts| is a sequence of
|ThreadGroup|s, then \SCALA{\|\| ts}\index[SCL]{\App} represents their
parallel composition, and is itself a |ThreadGroup|.
...
We now consider how to test\index{testing} the concurrent implementation.  The
obvious approach is to run both the sequential and concurrent implementations
on the same integral, and test whether they give the same result.  In
particular, my approach is to generate random polynomials for the
function~|f|, and random values for |a|, |b|, |n| and |numWorkers| (within
sensible ranges).  We can then run many such tests.
...
different orders, and so added up in different orders, sometimes giving
different results.
