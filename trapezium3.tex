...
An alternative is to use smaller tasks, with more tasks than
workers\index{task}.
%
...
balancing\index{load balancing}.

This pattern is known as \emph{bag of tasks}\index{bag of tasks}: the
controller holds a bag of tasks, which it distributes to workers.  (Sometimes
workers return sub-tasks to the controller, but that's not the case here.)
...
collector, communicating with the workers as illustrated in
Figure~\ref{fig:trapezium-bag-system}. 
%
...
\end{tikzpicture}
\end{center}
\caption{The system for the bag-of-tasks  example.}
\label{fig:trapezium-bag-system}
\end{figure}

%%%%%


We then construct the system as   in
Figure~\ref{fig:trapezium-bag-system}.   The resulting program can
