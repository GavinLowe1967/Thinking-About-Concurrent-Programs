...
object Factorial{
  /** The factorial of £n£.  Pre: £n $\ge 0$£. */£\label{line:fact-comment}£
  def fact(n: Int): Int = {
    require(n >= 0, s"Illegal argument $n for fact")
    var x = 1 // Invariant: £$\sm x = \sm i !$£ at the end of each iteration.£\label{line:inv}£
    for(i <- 1 to n) x = x*i
    x
  }
...
%% Most of the work is done by the function |fact|.
%% (Scala box~\ref{sb:functions}):
%% this takes an argument~|n| which is an |Int| (Scala
...
a recursive function, such as |fact|.  Even when it is not necessary, it is
often a good idea to include it, to help document the function.
...
The function~|fact| takes an argument~|n| which is an |Int|, and returns the
factorial of~|n|, also an |Int|.
...
The commands |require(|$test$|)| and |require(|$test$|, |$msg$|)| are similar,
except conventionally they are used to check preconditions of functions.  Such
statements are useful to catch when a function has been erroneously called
with its precondition not satisfied.  They are also useful to document the
code, stating assumptions.  Thus |fact| starts by checking the
precondition~$\sm n \ge 0$.
%% \end{scalaBox}
...
infers this type because |x| is initialised to~|1|, which is an |Int|.  
%% In Scala (and most other languages), these |Int|s can hold only numbers in
%% a limited range, from $-2^{31}$ (inclusive) to $+2^{31}$ (exclusive).
%
...
A |for| loop \SCALA{for(x <- xs) body} executes \SCALA{body}, where \SCALA{x}
takes each value in the sequence~\SCALA{xs} in turn.  The expression \SCALA{a
  until b} creates the sequence of numbers from~\SCALA{a} to~\SCALA{b-1},
inclusive.
...
%
Thus |fact| uses a |for|
loop, where the variable~|i| takes each value in the range from |1| to |n|,
inclusive, in turn.
%
...
%% \label{sb:assignment}
Variables are updated using |=|.  For example, in |fact|,
\begin{scala}
  x = x*i
\end{scala}
...
Thus the overall effect of the |for| loop is to multiply~|x| by each integer
...
\end{scala}
%% \end{scalaBox}
Thus the body of |fact| consists of the sequential composition of the four
commands on successive lines.

After the |for| loop of~|fact|, |x| equals the factorial of~|n|.  Finally, the
function returns the value of~|x|.

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
 
A one-line comment may be written in code by preceding it with ``|//|''.
Line~\ref{line:inv} includes a comment describing a loop invariant: a property
that is true at the end of each iteration of the loop. 
...
conventional to start each subsequent line with another ``{\scalastyle\rm
  *}''.  Thus line~\ref{line:fact-comment} contains a Scaladoc comment,
...
values, functions, types, and nested objects.  Typically, an object
encapsulates some data and some operations on that data.
%
The definition defines a single object |MyObject|: this is sometimes called
the singleton pattern.  The factorial program is
encapsulated in an object called |Factorial|.  
%
...
polymorphism, which we describe in Section~\ref{sec:scala-types}.)
...
The |Factorial| program has a |main| function which expects to receive a
single argument on the command line.  It converts that string into an |Int|,
...
%% \label{sb:print}
The standard function |print| prints its argument on the screen.  The function
|println| is similar, except it also ends the line.  Thus the |Factorial|
program calls |fact|, instantiating its parameter with~|n|, and prints the
result.
%% \end{scalaBox}
