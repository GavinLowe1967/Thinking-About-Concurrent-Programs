...
linearizability.\index{testing}\index{linearizability testing}

The basic idea is as follows. We run some worker threads on the concurrent
datatype, performing random operations, and record the history of operation
calls and returns.  We then search for a corresponding sequential history that
explains linearizability, and that is a valid history for the corresponding
sequential datatype.  If none is found, we signal an error.  We can repeat
...
be used without understanding the underlying algorithms.  The linearisability
testing framework encapsulates most of the boiler-plate code involved in the
testing: the programmer only has to provide the essentials, the sequential
specification object and  the worker threads.
...
object\index{sequential specification object}.  The underlying linearizability
framework requires this to be \emph{immutable} (unlike in
Section~\ref{sec:linearization}): the state of the specification object should
not change; instead, operations produce new instances of the same type.  The
framework also requires the specification object to be deterministic.  Here,
we use an immutable queue from the Scala API; we define |SeqQueue| as a type
synonym.
...
The worker takes a log object |log|, of type
|LinearizabilityLog|\SCLIndexX{linearizabilityLog}{LinearizabilityLog}, as a
parameter.  Each operation is performed and logged via a call to |log| (more
precisely, the |apply| function of~|log|; see Section~\ref{sec:scala-misc}),
...
function (see Section~\ref{sec:scala-misc}) to define the operation on the
...
Line~\ref{line:construct-lin-tester} constructs a
|LinearizabilityTester|\SCLIndexX{linearizabilityTester}{LinearizabilityTester}
object~|tester|.  This takes two type parameters, representing the type of
sequential specification objects and concurrent objects, respectively (we
could omit these type parameters, but I find it useful to include them).  The
|LinearizabilityTester| takes value parameters representing the specification
object, the concurrent object to be tested, the number of worker threads to
run (here~4), and the definition of a worker.
%
...
\begin{instruction}
Study the details of |QueueTest|.
\end{instruction}
...
values.  In fact, the |enqueue|s were concurrent, so could have been
linearized in either order, and so either of their values would have been
allowed.
...
We mentioned above that the problem of deciding whether a given history is
linearizable is NP-complete in general\index{linearizability
  testing!NP-completeness}.  Let's prove that fact, as it's quite easy.  This
proof is due to Anish Athalye~\cite{Athalye}.  We assume at least a passing
familiarity with NP-completeness.

Clearly the problem is in NP: given an interleaving of the history~$h$ with a
corresponding history~$h_s$ of the specification object, we can check that
$h_s$ is indeed a linearization of~$h$ in polynomial time.

To show the problem is NP-hard, we perform a reduction
from the subset-sum problem, which is as follows:
\begin{quote}
\textbf{Subset-sum problem:} Given a set $S \subseteq \sm{Int}$, and an
|Int|~$t$, determine whether there is a subset of~$S$ whose sum is~$t$. 
\end{quote}
%
This problem is known to be NP-complete.  

We will use the following sequential specification object.
\begin{scala}
object Spec{
  private var sum = 0
...
\end{scala}
(If using the linearization testing framework, we would have to produce an
immutable version.)
%% \begin{scala}
%% case class ISpec(sum: Int)
%% def seqAdd(s: ISpec)(x: Int) = ((), ISpec(s.sum+x))
%%   def get
%

Given an instance $S = \set{x_1,\ldots,x_n}$ and~$t$ of the subset-sum
problem, build the history
\begin{eqnarray*}
h & = & \langle\,
  \begin{align}
  \call.\sm{add}^1(x_1),\, \ldots,\, \call.\sm{add}^n(x_n),\, 
  \call.\sm{get}^0,\,  \return.\sm{get}^0\::t, \\
  \return.\sm{add}^1\::(),\, \ldots,\, 
     \return.\sm{add}^n\::() \, \rangle
  \end{align}
\end{eqnarray*}
%
\begin{figure}[hbt]
...
\draw[|-|] (0,0) -- node[above] {\scalashape add(1)} (7,0); \draw(4.5,0)\X
\draw[|-|] (1,-1) -- node[above] {\scalashape add(2)} (6,-1); \draw(1.5,-1)\X
\draw[|-|] (2,-2) -- node[above] {\scalashape add(3)} (5,-2); \draw(2.5,-2)\X
\draw[|-|] (3,-3) -- node[above] {\scalashape get: 5} (4,-3); \draw(3.5,-3)\X
\end{tikzpicture}
\end{center}
\caption{Timeline for the subset-sum example.}
\label{fig:subset-sum}
\end{figure}%
%
This is illustrated in Figure~\ref{fig:subset-sum} for the case $S =
\set{1,2,3}$ and $t = 5$.
%
Then $h$ is linearisable with respect to~|Spec| precisely if the subset-sum
problem should return |true|: if $h$ is linearisable, then $t$ equals the sum
of the $x_i$ values whose |add| operation is linearised before the |get|.  In
Figure~\ref{fig:subset-sum}, linearisation points are indicated by~|X|s; this
is a correct linearisation, because $2+3 = 5$, indicating a solution to the
subset-sum problem in this case.  If we could decide linearisability in
polynomial time, we could also decide the subset-sum problem in polynomial
time; hence deciding if a history is linearizable is NP-hard.
