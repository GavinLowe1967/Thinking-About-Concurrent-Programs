...
of~|log|; see Section~\ref{sec:scala-misc}), taking three parameters:
%
\begin{itemize}
\item
...
linearizable is NP-complete in general.  Let's prove that fact, as it's quite
easy.  (We assume here at least a passing familiarity with NP-completeness.)
...
$h_s$ is indeed a linearization of~$h$ in polynomial time.  To show the
problem is NP-hard, we perform a reduction from the subset-sum problem, which
is as follows:
\begin{quote}
...
\end{quote}
%
This problem is known to be NP-complete.  We will use the following sequential
...
\end{scala}
So, given an instance $S = \set{x_1,\ldots,x_n}$ and~$t$ of the subset-sum
problem, build the history
\begin{eqnarray*}
h & = & \langle\,
  \begin{align}
  \call.\sm{add}^1(x_1),\, \ldots,\, \call.\sm{add}^n(x_n),\, \call.\sm{get}^0, \\
  \return.\sm{add}^1\::(),\, \ldots,\, 
     \return.\sm{add}^n\::(),\,  \return.\sm{get}^0\::t \, \rangle
  \end{align}
\end{eqnarray*}
%
...
of the $x_i$ values whose |add| operation is linearised before the |get|.  If
we could decide linearisability in polynomial time, we could also decide the
