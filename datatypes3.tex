...
linearizable is NP-complete in 
general\index{linearizability testing!NP-completeness}.
Let's prove that fact, as it's quite easy.  (We assume here at least a passing
familiarity with NP-completeness.)  Clearly the problem is in NP: given an
interleaving of the history~$h$ with a corresponding history~$h_s$ of the
specification object, we can check that $h_s$ is indeed a linearization of~$h$
in polynomial time.  
