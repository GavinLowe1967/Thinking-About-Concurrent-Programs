...
In the recursive case, a worker returns one subtask to the bag, and continues
...
variables |fa| and |fb| with the invariant that if |a < b| then \SCALA{fa =
  f(a)} and \SCALA{fb = f(b)}.  We could extend this to the result of each
trapezium rule calculation, adding to each tuple a field \SCALA{area =
  (fa+fb)*(b-a)/2}, and again avoiding recalculations on each iteration of the
above loop.
\end{answerI}
