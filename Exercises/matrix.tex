...
To the right, for most values of~|n|, the program gets slower when $\sm n^2 /
\sm{taskSize} < 8$, because the number of tasks is less than the number of
workers.  However, for $\sm n = 64$, there is little difference, as the
savings on communication costs roughly balance the extra computational cost
for the workers that receive a task.
\end{answerI}
