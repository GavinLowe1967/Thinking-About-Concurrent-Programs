...
list.  The implementation uses a dummy header node, referenced by |head|; when
the queue is non-empty, the first element is stored in |head.next|.  It also
maintains a reference |last| to the last node in the list.

The implementation is correct when the queue is used sequentially.
...
\item Suppose threads~$t_1$ and~$t_2$ call |enqueue| concurrently, enqueueing
  ...
  before~$t_1$, then |last| will end up pointing to~$n_1$, so |last| will no
  longer be reachable from |head|, breaking an intended invariant; this means
  that the results of all subsequent enqueues will be lost.
