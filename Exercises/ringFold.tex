...
\begin{scala}
class RingFold[T: scala.reflect.ClassTag] ...
\end{scala}
%
...
\]
i.e.,~those elements combined together using~$f$ (functional programmers will
recognise this as a fold).  
...
%% \]
to node~$me+1$, where all indices are interpreted mod $n$. 
%%  The code has an
...
The changes from the previous code are below.  The code has an invariant that
at the start of round~|r|,\, $\sm y = \mathrm{combine}
\linebreak[1]\ f \linebreak[1]\ \seq{x_{\ss{me}-\ss{r}},  \ldots,
  x_{\ss{me}}}$.  The channels need to be buffered to avoid deadlocks, obviously.
%
\begin{scala}
...
value on its round |r| (since the channels are first-in first-out). 
\end{answerI}
