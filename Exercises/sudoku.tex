...
chapter.  We will consider a search algorithm implementing the abstract class
|DFGraphSearch| in Figure~\ref{fig:dfs-seq}.
%
%% %\begin{mysamepage}
%% \begin{scala}
...
\end{scala}
\caption{Depth-first search, and a sequential implementation.}
\label{fig:dfs-seq}
\end{figure}

Figure~\ref{fig:dfs-seq} also gives code for sequential depth-first search,
using a stack.  This version does not keep track of nodes seen previously:
there is no need in our program to solve sudoku problems, since the same node
will not be encountered twice.
