...
experiments showed that the latter approach is about 20 times slower!  The
reason for this is that the RNG executes operations in a one-at-a-time manner,
so as to be thread-safe.  This means that it acts as a bottleneck: each thread
has to wait its turn to use it.  Further, the state of the RNG has to be
copied to the cache of the relevant worker for each random number, and this is
slow.  When each worker has its own RNG, this bottleneck no longer exists.
