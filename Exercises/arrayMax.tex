...
and its index, using |numWorkers| worker threads.  You may assume that the
length of the array is at least |numWorkers|.
%
\begin{scala}
object ArrayMax{
  def apply(a: Array[Int], numWorkers: Int): (Int, Int) = {
    require(a.length >= numWorkers); ... 
  }
}
\end{scala}
%
...
\begin{answerS}
My answer is below.  The combining barrier receives from each thread a pair
containing the maximum value the thread found, and its index. 
%
\begin{scala}
