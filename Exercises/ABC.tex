...
the |B|- and |C|-threads call |BSync| and |CSync|, respectively, and receive
back the identities of the threads with which it synchronises.  Each operation
will block until other threads call the other two operations.  Implement a
class that extends this trait, using conditions.
...
class ABCSyncMonitor[A,B,C] extends ABCSync[A,B,C]{
  ...
  def ASync(me: A) = lock.mutex{
    aStart.await(!aFull)          // Wait for previous round to end (A1).
    assert(!bFull && !cFull); a = me; aFull = true
    bStart.signal()                // Signal to B£-£thread at (B1).
    aSignal.await()                // Wait for other threads (A2).
  ...
  def BSync(me: B) = lock.mutex{
    bStart.await(aFull && !bFull)         // Wait for my turn (B1).
    assert(!cFull); b = me; bFull = true
    cStart.signal()                 // Signal to C£-£thread at (C1).
    bSignal.await()                // Wait for other threads (B2).
    assert(aFull && bFull && cFull)
  ...
  def CSync(me: C) = lock.mutex{
    cStart.await(bFull && !cFull)         // Wait for my turn (C1).
...
One way to test the code is to run threads with distinct identities, the same
number of threads of each sort, that each call the operation once, writing
events into a log before and after the operation.  Then we can traverse the
log and check that all three threads agree on each synchronisation.
\end{answerI}
