...
|value| holds the value currently being sent, or maybe the previous value sent
(we will sometimes refer to a variable like this as a \emph{slot}).  The
variable |full| indicates whether the value in |value| is valid, i.e.~if a
current sender is trying to send it.
...
\begin{figure}
\begin{scala}
class SyncChan[A] extends SyncChanT[A]{
...
\item The receiver will then signal back to the sender on |continue|,
  indicating that it can continue; this makes the channel synchronous;

\item The receiver will signal to a sender for the next round on
  |slotEmptied|, indicating that it has cleared the value, so that sender can
  deposit its value.
\end{itemize}
%
...
at this point, as this thread cannot be preempted by another sender.
...
Note that for each operation, the |full| variable is needed for two slightly
different purposes.
%
\begin{itemize}
\item When the operation starts, the value of |full| indicates whether the
  operation can proceed immediately, without waiting.  In the case of
  |receive|, it might be that a sender has deposited a value, but the signal
  on |slotFull| was lost, because there was no waiting receiver at that
  point.  Likewise, in the case of |send|, it might be that the previous
  receiver has cleared the slot, but the signal on |slotEmptied| was lost,
  because there was no waiting sender at that point.  In each case, the value
  of |full| reflects the earlier signal. 

\item When the thread receives a signal, the value of |full| indicates whether
  the thread can now proceed, or whether it has been preempted and so must
  wait again.
\end{itemize}
%
It is common to use a boolean variable in combination with a |Condition| in
this way at the start of an operation.
...
When designing a synchronisation object like this, it is good to have a clear
plan as to the order in which different steps happen.  I find it useful to
draw a sequence diagram, like that in Figure~\ref{fig:syncChan-seqDiagram}:
this helps to clarify my thinking, and also acts as good documentation.  It is
normally best to have a rigid plan with a single possible order, rather than
to allow branching and so multiple different orders: this means there is less
to think about, and makes it easier to check that the code indeed enforces the
chosen order.
%
I like to compare this process with choreography: a group of dancers needs to
follow a clear plan of steps, to avoid treading on each other's toes; and a
group of threads needs to follow a clear plan of steps, to avoid disrupting
one another.
...
identify matching sends and receives in the log(if they exist).  We then check
that each invocation is matched by an invocation of the other operation, and
