...
A common way to coordinate multiple threads is via a ring\index{ring
topology}, where each thread communicates with just its two neighbours (see
...
common to use the word \emph{token}\index{token} for a message sent in a ring.
...
%
\[\mstyle
\textstyle\sum \set{x_j \| 0 \le j \le i}.
\]
When the value gets back to node \SCALA{0} it equals the overall sum, 
\[\mstyle
\textstyle\sum \set{x_j \| 0 \le j \le \sm n-1}.
\]
...
token back to node~$0$, since $0$ already has the sum.
%
\begin{instruction}
...
%
\begin{eqnarray*}
\sm{sum} & = & \textstyle\sum \set{x_{(i-j)\bmod \ss n} \| 0 \le j \le k}.
\end{eqnarray*}
%
...
More precisely, we arrange the nodes into a binary \emph{heap}\index{binary
heap topology}: see Figure~\ref{fig:interacting-peers}(d).  Node~$0$ is the
root of the tree.  Node~$i$ is the parent of nodes $2i+1$ and $2i+2$ (if those
nodes exist).
...
%
\[\mstyle
\textstyle\sum \set{x_j \| j \in descendents(i)}
\]
...
& x_i 
  \begin{align}
  \null +  \sum \set{x_j \| j \in descendents(2i+1)} \\
  \null +  \sum \set{x_j \| j \in descendents(2i+2)}.
  \end{align}
\end{array}
\]
