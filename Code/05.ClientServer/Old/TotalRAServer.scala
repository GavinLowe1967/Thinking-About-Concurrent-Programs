...
object TotalRATest{
  ...
  def client(me: ClientId, resourceServer: TotalRAServer, log: Log[LogEvent]) 
  = thread(s"client $me"){
  ...
  /** Run a single test. */
  def runTest(
      resourceServer: TotalRAServer, numClients: Int, numResources: Int) = {
  ...
  def main(args: Array[String]) = {
    ...
    for(r <- 0 until reps){
      val resourceServer = new TotalRAServer(numResources)
