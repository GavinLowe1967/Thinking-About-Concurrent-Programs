...
Figure~\ref{fig:bag-of-tasks-experiment}\index{tuning} shows timing results
for the bag-of-tasks example.  Each plot considers a particular choice for the
number of intervals, |n|, and the number of tasks, |nTasks|.  The x-axis
considers different numbers of workers, and the y-axis gives the times taken.
Each run used unbounded buffered channels.  Each observation performed
$2^{27}/\sm{n}$ runs.
...
become a bottleneck\index{bottleneck}.  However, for higher values of this measure, the program
scales better.  
...
bottleneck\index{bottleneck} again.
