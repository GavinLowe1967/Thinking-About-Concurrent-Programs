...
An \emph{exchanger} is a concurrent object that extends the trait in
Figure~\ref{fig:ExchangerT}.  Its main operation allows a pair of threads to
exchange values: each thread should pass in a value, and receive the other
thread's value back.
This exchange represents a \emph{synchronisation}: the two executions of
|exchange| must necessarily overlap in time.   (The trait includes an
