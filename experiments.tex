...
\emph{observations}\index{observation} (i.e.,~perform multiple runs), and then
...
\emph{significance level}\index{significance level}.  The value $1-\alpha$ is
known as the \emph{confidence level}\index{confidence level}, often expressed
as a percentage.  In the experiments described in the body of the book, we
took $\alpha = 0.05$, so the confidence level was $95\%$.  We want a
\emph{confidence interval}\index{confidence interval} $[m-s, m+s]$ such that
