...
\emph{modularisation}\index{modularisation}: splitting a large program up into
manageable chunks.  The most common modularisation technique is object
orientation, where objects encapsulate some data and operations on that data.
However, concurrency is also a form of modularisation, where each thread holds
some data, and communicates the results of operations on that data with other
threads.
...
Rust~\cite{rust} supports concurrency in a number of styles, including message
  passing.
%\footnote{ \url{https://doc.rust-lang.org/book/ch16-02-message-passing.html}}. 
