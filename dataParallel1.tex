...
\emph{rounds}\index{rounds}; in each round, each thread performs some
computation.  However, at the end of each round, each thread needs to wait for
the other threads to finish that round before they all proceed to the next:
this requires a global synchronisation\index{global synchronisation}.
...
\emph{barrier synchronisation}\index{barrier synchronisation}, since it
represents a barrier than no thread 
may pass until all have reached that point.

Suppose we have |p| threads with identities $\interval{\sm 0}{\sm p}$.  Then a
suitable barrier synchronisation object may be created in SCL
by:\SCLIndexX{barrier}{Barrier} 
%
\begin{scala}
  val barrier = new Barrier(p)
\end{scala}
%
A thread with identity~|me| performs the barrier synchronisation by
executing\SCLIndex{sync}% 
%
\begin{scala}
  barrier.sync(me)
\end{scala}
...
from each of the |p|~threads, and then sends them each a message on
channel~|leave|, telling them they can continue.
...
cache consistency\index{cache consistency} between different threads.  If one
thread writes to a shared variable before a synchronisation, and another
thread reads that variable after the synchronisation, then the read is
guaranteed to see the effects of the write.  Further, compiler optimisations
are not allowed to break this property.
...
bodies in each step of the simulation (we expect there to be many more bodies
than workers).  However, each worker will need to read the states of all of
the bodies.  We will use barrier synchronisations to coordinate the threads
and to avoid races.
...
The force on each body gives an acceleration equal to the force divided by the
body's mass.
...
threads have finished the calls to |updateVel|, but before any call to |move|.
That means that each call to |updateVel| will depend on the previous positions
of the bodies, as in the sequential program.  Two calls to |updateVel| on
different bodies may happen in either order; but these operations do not
produce a race.  And likewise two calls to |move| on different bodies may
happen in either order; but these again do not produce a race.
