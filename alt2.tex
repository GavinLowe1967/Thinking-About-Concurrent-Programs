...
The Dining Philosophers\index{Dining Philosophers problem} problem is an
example formulated by Edsger Dijkstra to illustrate some of the problems that
can arise in concurrent programming~\cite{dining-phils}.  It is presented in
terms of interactions between people; but it is an analogy for concurrent
threads or processes sharing resources.
...
as soon as it's available (again, they might need to wait).  Once the
philosopher has two forks, they serve themself and spend some time eating.
The philosopher then puts the forks down, leaves the table, and does some more
thinking.
...
|assert| statements check the correct messages are received.  Using
assertions\index{assertion} like this is good practice.  If nothing else, it
acts as good documentation.  But such assertions can also help to catch
mistakes.  (I have found lots of mistakes in my own code by including
assertions like these.  If I had omitted those assertions, it would probably
have led to errors arising later in the program, but it would have been much
harder to identify the cause of the problem.)
...
When we run the system, sometimes it deadlocks\index{deadlock} almost
immediately.  A~typical deadlocking trace is:
%
\begin{scala}
...
this point, each philosopher is trying to pick up their right fork, i.e.~to
send a |Pick| message on their |right| channel; however, the corresponding
fork is not willing to receive that message.  This means that the system is
deadlocked.
...
tracking down bugs.  Normally, a better way to is to use logging.\index{logging}

The SCL class |Log|\SCLIndexX{log}{Log} provides log objects.
%
...
thread's identity as a parameter: to identify the relevant thread-local log to
use.  Different threads should use different identities, in the relevant
range.  Each logged value is stored in the relevant thread-local log, together
with a timestamp (more precisely, the time elapsed since the |Log| object was
created, to avoid problems with timestamp wrap-around).
%
...
must use Windows, the class |SharedLog|\SCLIndexX{sharedLog}{SharedLog}
provides the same interface.  However, it will give worse performance.
Further, it might affect the likelihood of detecting bugs: I have experienced
bugs that would appear when no logging was performed, or when the
timestamp-based log was used; but logging with something equivalent to
|SharedLog| affected the speed of threads sufficiently that the bug no longer
appeared in a reasonable time!
