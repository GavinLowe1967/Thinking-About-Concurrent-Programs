\section{Implementing an Exchanger Using Conditions}
...
We use a |Lock| to provide mutual exclusion, together with two |Condition|s
...
thread can now start an exchange (unless it has been preempted by another
thread).  The condition |swapped| is used by the second thread to signal that
it has swapped values.  Figure~\ref{fig:exchanger-seqDiagram} illustates the
sequence of signals.  
