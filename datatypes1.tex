A \emph{concurrent datatype}\index{concurrent datatype} is a datatype, for
example, a queue, stack, set or mapping, that can be safely accessed
concurrently by multiple threads.  Operation executions should appear to take
place in a one-at-a-time order, without interference.

Threads can use the concurrent datatype much as they would with a sequential
datatype: the implementer of the threads does not need to think much about
concurrency.
...
\emph{Datatype-based concurrent programming}\index{datatype-based concurrent
programming}  encapsulates all, or nearly all, the
...
objects can be either \emph{total}, i.e.,~able to be performed in an arbitrary
...
the queue is nonempty; but then other threads might empty the queue before
$t$ attempts the |dequeue|, at which point the precondition is violated.  This
is a \emph{time-of-check to time-of-use}\index{time-of-check to time-of-use}
(TOCTTOU) problem: there is a delay between the thread checking the relevant
precondition and performing the operation that depends upon it, during which
the precondition becomes false; several exercises in Chapter~\ref{chap:intro}
...
when the precondition does hold.  Another alternative is for the operation to
throw an exception, and expect the thread to catch it.  Recall that operations
that treat preconditions in this way are called \emph{total}: they can take
effect in any state.
...
We implement a total concurrent queue,\index{total queue} with the following
interface, where the |dequeue| operation returns an |Option| type, with |None|
used to indicate am empty queue.
%
%\begin{mysamepage}
\begin{scala}
/** A total queue. */
...
stores the queue itself (using a mutable |Queue| from the Scala API)\@.
Clients use channels to cause the server to enqueue and dequeue values.  Note
that the server handles operations in a one-at-a-time way, preventing
...
We include a |shutdown| operation, to provide a way to terminate the server
thread, and so allow garbage collection.  Later, we will see other
implementations of |TotalQueue|.  We include |shutdown| in the |TotalQueue|
trait, so as to ensure a uniform interface, for example, for testing.  It is
convenient to give |shutdown| a default no-op implementation in |TotalQueue|,
as that is what we will want in later implementations, and to override the
definition just in |Server|\-|TotalQueue|.
