...
In this chapter we have studied a data parallel problem\index{data
parallel}.  The approach to such problems is to split the input data
between different worker threads, and to combine the results together.  Many
compute-intensive problems fall into this class, so it is a very useful
pattern.  It requires that there are meaningful ways to split the data between
workers, and to combine the results efficiently.
...
perhaps using different concurrency techniques: we will see another
implementation of the bag-of-tasks object, using an explicit lock, in
Section~\ref{sec:concurrent-objects-locks}.
