...
The deadlock\index{deadlock} in the Dining Philosophers example corresponds to
a cycle of waiting.  Each philosopher is trying to pick up their right-hand
fork, and is waiting for that fork.  Each fork is waiting for their right-hand
philosopher to put it down.  Let's write $t_1 \waitingFor t_2$ to signify that
thread $t_1$ is blocked, waiting for thread~$t_2$.  Then we have the following
cycle of waiting:
\[\mstyle
...
system based on message passing.  Suppose that for every channel~$c$, there is
some thread that sometimes sends on~$c$, and some thread that sometimes
...
this infinite sequence must contain the same thread twice.  Thus this sequence
contains a waiting cycle.
...
Within SCL, this could be achieved using a timeout\index{timeout}.  Each
channel has an operation\SCLIndexX{sendWithin }{sendWithin}
%
\begin{scala}
...
the send was successful.  Likewise there is an
operation\SCLIndexX{receiveWithin }{receiveWithin}
%
\begin{scala}
...
returns a result of type |Option[A]| (see Section~\ref{sec:scala-types}): if
it successfully receives a value~|x|, it returns~|Some(x)|; otherwise it
returns~|None| to indicate a timeout.  There are also operations
|sendWithinNanos|\SCLIndex{sendWithinNanos} and
|receiveWithinNanos|\SCLIndex{receiveWithinNanos} where the time is given in
nanoseconds.
...
syntax for a branch:%
% Hacked index entry for =!=>
\index[SCL]{\ignoreX {{=}}\protect\SCALA{=}\Ashriek\SCALA{=>}}
%
\begin{scala}
  bool && out =!=> { expression }
\end{scala}
...
using a continuation, with the following syntax.\SCLIndex{==>}
%
\begin{scala}
   bool && outport =!=> { expression } ==> { command }
\end{scala}

Here's another definition for the |tee| function, which produces a thread that
inputs on an in-port, and then outputs on two out-ports in either order.
%
%\begin{mysamepage}
\begin{scala}
...
\begin{scala}  
  def buff2[T](in: ??[T], out: !![T]): ThreadGroup = {
    def empty(): Unit = { val x = in?(); holding(x) }
    def holding(x: T): Unit = {
      alt( out =!=> { x } ==> { empty() } 
         | in =?=> { y => out!x; holding(y) } )
    }
...
%% \end{scala}
%
The function |empty| corresponds to when the buffer is empty: it can input a
value~|x|, and move to the state |holding(x)|.  The function |holding(x)|
corresponds to when the buffer is holding the single value~|x|: it can
output~|x| and become empty; or it can input another value~|y|, at which point
it is full so must output~|x|, and subsequently it is holding just~|y|.  The
body of |buff2| runs a thread starting in the |empty| state, and catching
exceptions. 
