...
%
It would be better to encapsulate\index{encapsulation} each relevant design
decision within a single object, so that changes to a design decision will
involve just that object.

Each such object will be designed to allow concurrent calls of its methods,
and to avoid race conditions; we call it a \emph{concurrent
  object}\index{concurrent object}.  Reasoning about correctness will (mostly)
involve reasoning about a single concurrent object at a time: we can persuade
ourselves that a concurrent object is correct by considering it in isolation.
Code outside that object can use it without worrying about how it is
implemented.
...
|BagOfTasks| to get a task, and pattern matches on it (see
Section~\ref{sec:scala-misc}), exiting its main loop when it receives |null|.
Finally, it uses the |add| operation on the |Collector| to return its
subtotal.
...
workers, and then gets the final result from the collector.  In later
chapters, we will see other implementations of the two traits; adapting the
code to use these is  trivial.
...
\end{scala}
\caption{An implementation of {\scalastyle BagOfTasks} using message passing.}
...
%
More generally, if |t| is a |ThreadGroup|, then |fork(t)|\SCLIndex{fork} (or
|t.fork()|) starts a new thread (or threads) running which executes~|t|.
However---unlike with |run(t)|---the new thread runs in parallel to the
current thread.  In Figure~\ref{fig:bagOfTasksObject}, the construction of the
...
\end{scala}
\caption{An implementation of {\scalastyle Collector} using message passing.}
\label{fig:collector}
\end{figure}
