...
We now consider another example of fine-grained 
concurrency\index{fine-grained concurrency}.  We build a circuit to sort a
stream of integers, based on the 
Mergesort algorithm\index{Mergesort}.  The circuit inputs a steam of |Int|s
(with the end of stream signalled by the channel closing), and then outputs a
sorted version of that stream (and closes the channel to indicate the end of
the stream).  The circuit is not the most practical way to sort numbers, but
it illustrates various ideas.
...
\end{mysamepage}
%
The command |attempt{p}{q}|\SCLIndex{attempt} acts like~|p|, but if that
throws a |Stopped| exception\SCLIndexX{stopped}{Stopped}, it executes~|q|.
Thus if the initial attempt to input |x1| on |in| fails, the function signals
|endOfStream| on |out|; and if the attempt to input~|x2| on~|in| fails, the
function outputs~|x1| and then signals |endOfStream| on |out|.
...
We now consider testing\index{testing}.  Testing is an important part of
programming.  But testing is particularly important for concurrent programs,
because they tend to be somewhat harder than sequential ones.
...
mean that $a$ necessarily happens 
before\index{happens before ($\preceq$)}~$a'$.  For example, $a$ and~$a'$ might
be events performed by 
the same thread in that order; or they might, respectively, be a send and
receive on a buffered channel.
