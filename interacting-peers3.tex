...
We want a protocol to elect one node as a leader\index{leadership election}
(or controller) for subsequent computation.  More precisely, each node should
end up with a boolean indicating whether it is the leader: the leader could
then inform the others of its own identity, if that is required.  We will
encapsulate solutions into classes with the following interface, where a
thread with identity~|id| receives ring messages on~|in|, and sends ring
messages on~|out|. 
%
\begin{scala}
...
ones that are still candidates to become the leader, while those relaying
