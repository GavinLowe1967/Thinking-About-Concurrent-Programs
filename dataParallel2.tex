...
\[\mstyle
\forall k \in \interval{0}{\sm n} \spot 
  \sm{sum}(k) = \textstyle\sum \sm{a}[0 \upto k] 
\]
Recall that we write $\interval{l}{r}$ to represent the half-open interval
from~$l$, inclusive, to~$r$, exclusive.  Here we write $\sm a[l \upto r]$ to
represent the closed segment of~|a| between indices~$l$ and~$r$, inclusive.
And below we write $\sm a (l\upto r]$ to represent the half-open segment
  excluding index~$l$ but including index~$r$.

The following simple sequential program calculates the segment sums.
\begin{scala}
...
/** Calculate prefix sums of array £a£ of size £n£ in £$\Theta(\log \sm n)$£ (parallel) steps.  */
class PrefixSums(n: Int, a: Array[Int]){
  ...
  private def summer(me: Int) = thread(s"summer($me)"){
    // Invariant: £$\sm{gap} = 2^{\ss r}$£ and £$\sm s = \sum \sm a\revint{\sm{me}-\sm{gap}}{\sm{me}}$£ (with fictious values
    // £$\sm a(i) = 0$£ for £$i < 0$£).  £r£ is the round number.
