...
Exercise~\ref{ex:dfs} asks you to implement a concurrent algorithm for
approximate depth-first search.
...
We now consider a concurrent algorithm.  Each worker thread behaves very much
as in the main loop of the sequential algorithm.
...
If a path is found, we need to arrange for that path to be returned. However,
...
found a solution and it trying to send it to the controller.
...
It is worth emphasising how the use of concurrent datatypes has simplified
...
termination in this case comes from the fact that there are two different ways
...
nodes at a particular distance from the start node is known as a
\emph{ply}\index{ply}.  A true breadth-first search would expand nodes in ply
order.  However, with our implementation, if a thread is delayed, deeper nodes
might be expanded before a node from an earlier ply.  This might mean that the
path that is found isn't the shortest.  In many cases, this won't matter much.
If true breadth-first behaviour is needed, the threads can performs a global
