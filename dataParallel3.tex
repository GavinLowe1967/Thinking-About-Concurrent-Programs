...
technique, which can be more efficient.  We start by describing the
technique.
...
A \emph{combining barrier}\index{combining barrier} is associated with a
function \SCALA{f: (A,A) => A}, which is assumed to be associative.  Each
thread contributes some piece of data~$x_i: \sm{A}$ to each synchronisation.
All the threads then receive back the result of combining all the $x_i$ values
together using~|f|, i.e.,
\[\mstyle
...
applications, |f| is commutative, and so the order doesn't matter.  For
example, if |f| adds its arguments, the result is the sum of all the~$x_i$.

In SCL, the expression \SCALA{new CombiningBarrier(p, f)}%
\SCLIndexX{combiningBarrier}{CombiningBarrier} creates a combining barrier
for |p| threads, associated with function~|f|.  In the Jacobi iteration
example, we want the combining barrier to calculate the conjunction of the
workers' |myDone| variables, so we can define the combining barrier
by\footnote{The notation {\scalashape \_ \&\& \_} uses placeholder notation;
  see Section~\ref{sec:scala-misc}.  It represents a function that takes two
  arguments, \SCALA{x} and~{\scalashape y}, and returns {\scalashape x \&\&
    y}.}:
%
\begin{scala}
...
\end{scala}
%
Each thread can then execute\SCLIndex{sync}
\begin{scala}
...
by |new AndBarrier(p)|\SCLIndexX{andBarrier}{AndBarrier}.  Likewise, an
|OrBarrier|\SCLIndexX{orBarrier}{OrBarrier} returns the disjunction of the
inputs.
...
\begin{figure}
\begin{scala}
/** A concurrent implementation of Jacobi iteration, using message passing. */
