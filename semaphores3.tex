...
We will now implement a simple synchronous channel---similar to that in
earlier chapters---using semaphores.  We will need three signals for each
...
that semaphore is initially in the ``up'' state).  
\[
...
s.\sm{up}_{n+1}$, so the precondition of the |up| operation is satisfied.

%%%%%

The signalling here is done slightly differently from that in the
