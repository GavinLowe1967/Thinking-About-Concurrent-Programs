...
nodes are reachable from the controller, and to find a spanning
tree\index{spanning tree} for them, i.e.,~a collection of edges that connect
those nodes, without any cycles.  This spanning tree could be used
subsequently to distribute messages.
...
the tree further.  When that node has grown the tree as much as possible, it
returns a message |Finished(edges)| to~|n|, where |edges| is the list of edges
in the subtree it has formed.  Node~|n| then accumulates all such subtrees and
...
the first.  
% (but it won't expect a subsequent |Finished| message).  
If node |n1| has previously received a |Start| message, and subsequently
receives a message |Start(n2)| from |n2|, then previously it sent a message
|Start(n1)| to~|n2|, which |n2| will also ignore.  In this case, neither |n1|
nor |n2| will reply to the other with a |Finished| message.  The edge between
|n1| and~|n2| will not be included in the spanning tree, since both nodes will
be connected to the initiator~|0| by other edges, so that edge would
erroneously create a cycle.

The above example also illustrates that the system needs to use buffered
...
Each node (including the initiator, node~|0|) then sends a |Start| message to
each of its neighbours in the graph other than its predecessor
(line~\ref{line:sending}).
...
list~|edges| (see Section~\ref{sec:scala-types}).  As discussed above, if it
...
the correctness of the result.  It starts by checking that the result really
does represent a tree: it traverses the list of edges, calculating (using
bitmap |reached|) which nodes are reached by the tree, and checks that each
edge joins a node already seen to a node not previously seen, and that each
such edge is an edge in the original graph.  Finally, it checks that no
further nodes could have been reached, i.e.~that the result does span all the
reachable nodes.

\begin{instruction}
Make sure you understand the program and the testing code.
\end{instruction}
