...
We want to test\index{testing} the resource allocation server.  The property
we want to test is that two clients never hold the same resource: if a
resource~$r$ is allocated to client~$c_1$, it can't be allocated to another
client~$c_2$ until $c_1$ has returned it.
...
We use a log\index{logging} of type |Log[LogEvent]|, where |LogEvent| is
defined in Figure~\ref{fig:RATest-1}.
...
\begin{scala}
  ...
  /** Run a single test. */
  def doTest(resourceServer: RAServer) = {
...
The function |doTest(resourceServer)| (Figure~\ref{fig:RATest-2}) performs a
...
log represents a correct execution.  Finally, it shuts down |resourceServer|, to
...
The program's |main| function calls |doTest| many times. 
...
create a new reply channel\index{reply channel} for each request, and to send
the reply channel within the request.  The server can then reply on this reply
channel.
%
...
\begin{scala}
  ...
  private def server = thread{
    ...
    // Invariant: if £pending£ is nonempty, then all entries in £free£ are false.
    var done = false
...
resource to the corresponding client.
...
when there is an unallocated resource: we say that it is a \emph{partial
  operation}\index{partial operation}.  By contrast, in the previous
implementations, the operation was a \emph{total operation}\index{total
operation}, that could take effect in any state.  (The terms ``partial'' and
``total'' are by analogy with partial and total functions, which are defined
on some or all potential arguments, respectively.)
...
The use of a queue then ensures a degree of fairness\index{fairness}.
Assuming resources are repeatedly returned, once a client attempts to obtain a
resource, its message on |acquireRequestChan| will eventually be received, so
the reply channel will be enqueued; and eventually the reply channel will
reach the front of the queue, and the request serviced.
...
be blocked\index{blocking} on the |acquireRequestChan| channel until there is
a resource free.  This can be achieved simply by adding a variable that keeps
track of the number of free resources, and a suitable guard on the
|acquireRequestChan| branch of the |serve|, to block communications when there
