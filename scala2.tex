...
\noindent
This executes the bytecode in the \texttt{\codecolour .class} file on the Java
Virtual Machine (JVM)\index{JVM}.  The array~|args| is instantiated as an
array holding any additional arguments provided on the command line (as
strings).
%% \end{scalaBox}
...
In Section~\ref{sec:scala-core}, we described the encapsulation of code in
(singleton) objects.  Alternatively, code may be encapsulated in classes,
traits, and abstract classes, which we now describe.

%% .  Scala differentiates between classes (of which
%% there may be many instances) and objects (of which there is a single
...
%% \label{sb:classes}
A class is a template for objects.  Thus Scala differentiates between classes
(of which there may be many instances) and objects (of which there is a single
instance).

A class can be defined using syntax
\begin{scala}
  class MyClass[£$\sm A_1, \ldots, \sm A_k$£](£$\sm x_1: \sm T_1, \ldots, \sm x_n: \sm T_n$£){ ... } 
\end{scala}
Here $\sm A_1, \ldots, \sm A_k$ are polymorphic type parameters (which we
describe in Section~\ref{sec:scala-types}), $\sm
x_1,\linebreak[1] \ldots,\linebreak[1] \sm x_n$ are parameter names, and $\sm
T_1, \ldots, \sm T_n$ are corresponding types (possibly using the type
parameters).
...
%
Companions can access |private| and |protected| fields (see below) of one
another.
%% \end{scalaBox}

%%%%%

Scala provides two mechanisms to capture the interface of objects, separate
from their implementation, and to encapsulate common code between different
objects: traits and abstract classes.

%% \begin{scalaBox}{Traits}
%% \label{sb:traits}
A trait or abstract class  can be defined using syntax 
\begin{scala}
  trait MyTrait[£$\sm A_1, \ldots, \sm A_k$£]{ ... }
  abstract class MyClass[£$\sm A_1, \ldots, \sm A_k$£](£$\sm x_1: \sm T_1, \ldots, \sm x_n: \sm T_n$£){ ... } 
\end{scala}
where $\sm A_1, \ldots, \sm A_k$ are polymorphic type parameters, $\sm
x_1, \ldots \sm x_n$ are value parameters, and $\sm T_1, \ldots, \sm T_n$ are
corresponding types.

%%  A trait typically defines the interface of one or
...
In either a trait or abstract class, some values or functions can be abstract:
the trait or abstract class contains their types, but with no definition;
instead, a subclass provides the definition (see below).
%
A trait or abstract class cannot be instantiated directly: instead subclasses
...
or within a companion object or class.
%% \end{scalaBox}
...
\item Pattern matching (which we describe in Section~\ref{sec:scala-misc}) can
  be used.  With the above definition, a pattern |C(pat|$_1$|, pat|$_2$|)|
  matches a value |C(x,y)| provided |x| and |y| match |pat|$_1$ and |pat|$_2$,
