...
programs can act in unexpected and undesirable ways.  In particular, it is
necessary to ensure disciplined access to shared variables.
...
We use the Scala Concurrency Library (SCL)\index{SCL} in this book.  It
provides a number of convenient concurrency primitives\index{concurrency
  primitive}.  An index at the end of the book lists the SCL primitives we
use.  

Line~\ref{line:input} of Figure~\ref{fig:race} imports the SCL library, so
the main primitives can be used directly; for example, ``|thread|'' is
shorthand for ``|ox.scl.thread|''.  (In later programs, we will tend to omit
this |import| statement, for brevity.)

A definition of the form \SCALA{thread\{<code>\}}\SCLIndex{thread} defines a
thread that when executed will execute \SCALA{<code>}.  For example,
line~\ref{line:p} defines |t1| to be a thread that when executed will
increment the shared variable~|x| 1000 times, and line~\ref{line:q} defines
|t2| to be a thread that when executed will decrement |x| 1000 times.  Each
thread is of type |ThreadGroup|\SCLIndexX{threae}{ThreadGroup}\footnote{This
  type is not the same as the type {\scalashape java.lang.ThreadGroup}.}.
%% Note: the index entry for ThreadGroup will appear after that for thread. 

%% Hack here to get the indexing to accept ||.  Note: the order of the index
%% depends on the name of the macro!
\def\App{\protect\SCALA{\|\|}}
If |p| and |q| are |ThreadGroup|s, then \SCALA{p || q} represents their
parallel composition\index[SCL]{\App}, also of type |ThreadGroup|.  For example,
line~\ref{line:system} defines |system| to be the parallel composition of~|t1|
and~|t2|.

If |p| is a |ThreadGroup|, then |run(p)| or |p.run|\SCLIndex{run} runs~|p|.  A
parallel composition terminates when all components have terminated.  Thus the
|main| function at line~\ref{line:main} runs |system|, i.e.,~it runs |t1|
and~|t2| in parallel; when both have terminated, it prints the final value
of~|x|.
...
Certain actions can be considered \emph{atomic}\index{atomic action}; for
example a single machine instruction; a Scala program runs on the Java Virtual
Machine (JVM)\index{JVM}, so we can consider each JVM instruction to be
atomic.
%%   or, in the case of a Scala program, a single instruction of the
%% Java Virtual Machine (JVM)\index{JVM} on which the program runs.
Example actions include loading the value of a variable into a
...
More generally, two actions are \emph{independent}\index{independent actions}
if their order may be reversed without changing the overall effect.  For
example, two actions concerning distinct variables are independent, as are two
reads of the same variable.  Likewise, the |INC| and |DEC| actions by the
above threads affect only thread-local registers, so are independent of any
other actions.  Interleaving of independent atomic actions is not problematic.

However, some actions are non-independent, for example two writes of the same
variable, or a read and a write of the same variable: different orders for
such actions lead to different outcomes.  Interleaving of non-independent
atomic actions can be problematic, as it means that the program can produce
different results on different runs.

We say that a program contains a \emph{race}\index{race} if two
non-independent actions can be interleaved, and one of the interleavings leads
to an incorrect result.  In particular, where those actions are reads or
writes of shared variables, we say that it is a \emph{memory
  race}\index{race!memory race}.  The
program in Figure~\ref{fig:race} contains memory races: the writes by the two
threads can happen in different orders, leading to different results, as in
the second and third interleavings above.
...
unpredictable.  
%
As described earlier, multiprocessor machines may cache variables\index{cache
  memory}.  Caching can make a program much faster; but it can create problems
...
concerns \emph{compiler optimisations}\index{compiler optimisation}.  The
compiler may optimise code, according to certain rules, to something that is
equivalent when run sequentially.  Compiler optimisations are certainly
useful, and often produce faster code.  However, the resulting code might not
be equivalent when run as part of a concurrent program.

Consider the code in Figure~\ref{fig:busyWait}.  This uses a variant of the
|thread|\SCLIndex{thread} function: the construct |thread(name){<code>}| acts
like |thread{<code>}| but gives the name |name| to the thread; we will see how
...
|done| on each iteration.  Now, this thread can read |done = false| and loop
forever: the program gets stuck!  In fact, on my machine, the program runs for
a few hundred iterations and then gets stuck: I think the just-in-time
compiler (which optimises Java bytecode to native machine code, while the
program is running) performs an optimisation equivalent to the one above.
...
the terminal produces a thread dump\index{thread dump}.  This lists
information about all the running threads, such as giving their current line
number in the code.  (This normally includes several threads that are part of
the runtime, rather than the program in question; these can normally be
ignored.)  In particular, this thread dump uses names given to threads by the
|thread(name){...}| construct.  In the example of the previous paragraph, |t1|
is not listed, because it has terminated; however, |t2| is still at the line
corresponding to the |while| loop.

The style of the loop in~|t2|, where the thread spins, waiting for a condition
to become true, is known as \emph{busy waiting}\index{busy waiting}.  This is
normally considered bad style.  As the above example shows, it often doesn't
work!  And even if it did work, the spinning thread would be consuming
computational resources that might be better used elsewhere.  It would be
better for the thread to suspend until the condition becomes true: we will see
in later chapters how to do this.
...
\emph{Safety properties}\index{safety} specify that the results produced by a
program are correct; informally, nothing bad happens.  By contrast,
\emph{liveness properties}\index{liveness} specify that the program does
actually make progress and produce results, and doesn't get stuck; informally,
something good happens.  We will be interested in both safety and liveness
properties.
