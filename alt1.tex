...
\emph{alternation}\index{alternation}, or \emph{alt}, construct does
that for us. 
%
In this chapter, we will study alternation: we start by describing the basic
syntax of alternation in SCL, and then use it in examples.

%%%%%

% Create index entry for "|".
\def\indexBar{\index[SCL]{\protect\ignoreX {{*}}\protect\scalastyle$\mid$}}

The following construct defines a thread that tries to receive from any one of
several different in-ports:\SCLIndex{alt}\SCLIndex{=?=>}\indexBar
%
\begin{scala}
...
corresponding branch is \emph{feasible}\index{feasible}.  If no branch is
feasible, the alt throws an |AltAbort|\SCLIndexX{alté}{AltAbort} exception (a
subclass of \SCALA{Stopped}\SCLIndexX{stopped}{Stopped}): this corresponds to
the case where the alt will never be able to communicate.
...
branch is \emph{ready}\index{ready}.
%
...
\end{scala}
%
Suppose no branch of the alt is feasible, which means that for every branch,
either the guard is false or the port is closed.  Then the alt will throw an
...
say that the |serve| is \emph{fair}\index{fairness} to each branch.
...
Fairness\index{fairness} crops up in a number of scenarios in concurrent
programming.
%
...
arbitrary sequence of branches~|bs|, denoted \SCALA{alt( \|(bs) )}\indexBar\  
and \SCALA{serve( \|(bs) )}.  For example, given an array |ins| of in-ports,
we can form a generalisation of the tagger as follows.  It is built from a
sequence of branches that is generated using a |for| expression.
% 
\begin{scala}
...
alternation like this as \emph{indexed}\index{alternation!indexed}.  The branch for
index~|i| is willing to receive on |ins(i)|; if it receives~|x|, it sends the
pair |(i, x)|.  This |serve| is fair to each in-port: if a particular in-port
is continually ready, it will be selected within at most |ins.length|
iterations.  The construct terminates when all the ports in |ins| are closed,
or |out| is closed.
