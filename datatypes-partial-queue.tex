...
%
An alternative is to block that operation until there is a value to be
dequeued.  This gives a partial queue, with the interface given by
|PartialQueue| in Figure~\ref{fig:partial-queue}\index{partial queue}.  Note
that if the queue is empty and all threads are attempting a dequeue, the
system will deadlock; we will return to this point later.
...
trait PartialQueue[T]{
  ...
  /** Dequeue a value.  Blocks until the queue is nonempty. */
  def dequeue(): T
...
We can test the partial queue for linearizability\index{linearizability
testing}, much as for the total queue.
Figure~\ref{fig:partial-queue-lin-tester} includes the main parts.  Again, we
can use an immutable queue as the sequential specification object.
...
operation now has a nontrivial precondition, namely that the queue is
nonempty.  We need to arrange that the linearizability tester does not
linearize such an operation when the precondition does not hold.  The way to
do this is to include a |require| statement (see Section~\ref{sec:scala-core})
in the corresponding sequential operation~|seqDequeue|.  This will throw an
...
|dequeue| to be linearized at this point.  Thus the |require| statement
captures the precondition. 
...
exception).\index{terminating partial queue}
...
We can apply linearizability testing\index{linearizability testing} to the
terminating partial queue.  Figure~\ref{fig:terminating-queue-tester} gives
the main parts; other parts are standard.
...
contents~|q|, and |None| representing a queue that has terminated.  

The function |seqEnqueue| is the enqueue operation on the sequential
specification object.  The definition is straightforward when the queue is not
terminated.  However, we should not reach a state where an |enqueue| is
attempted once the queue has terminated: this would represent a bug in our
testing code, so we throw an exception.

Likewise, the function |seqDequeue| is the dequeue operation on the
specification object.  The definition is straightforward when the queue has
not terminated and is nonempty.  If a |dequeue| happens when the queue is
empty, we treat this as the termination point, so the |dequeue| returns
|None|, and the specification object enters the terminated state.
Subsequently, other |dequeue|s also return |None|, and the specification
object remains in the terminated state.

Each worker repeatedly enqueues with probability~$0.3$, and otherwise
dequeues.    The expression 
\begin{scala}
  q => {val res = q.dequeue(); done = (res == None); res}
\end{scala}
is an anonymous function (see Section~\ref{sec:scala-misc}) that takes a
terminating partial queue~|q|, and returns the result of |q.dequeue()|, but
has the side effect of setting |done| to true if this result is |None|.  Thus
the worker terminates when |None| is received from a |dequeue|.
...
%% (with the actual implementation, those subsequent calls would throw a
%% |Closed| exception, but a different, faulty implementation might act
%% differently).
