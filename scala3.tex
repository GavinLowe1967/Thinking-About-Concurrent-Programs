...
%%  or reference types (Scala
%% box~\ref{sb:reference-types}).  Scala supports parametric polymorphism (Scala
%% box~\ref{sb:polymorphic}) and function types (Scala
...
We described value types in Section~\ref{sec:scala-core}).  By contrast, each
class defines a type with the same name, which is a \emph{reference type}.
Each variable of that type actually holds a reference to the object, i.e.~the
...
|Int| to |Boolean|.  (In fact, each such function is implemented as a
Scala object, implementing a trait |Function|.)
%% \end{scalaBox}
...
types, including arrays and strings (each described in
Section~\ref{sec:scala-core}).  We describe two more such types here.
...
|Some(x)| and |None| can be used as patterns in pattern matching (see
Section~\ref{sec:scala-misc}).  The pattern ``|Some(x)|'' (where |x| is a
...
For other API classes, you should refer to the online documentation (e.g.~by
searching for ``Scala API''; recall that this book uses Scala~2.13).
