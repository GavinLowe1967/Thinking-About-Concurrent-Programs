...
communicate only by sending and receiving messages
over \emph{channels}.  In this book, we will be dealing with
threads; but the same techniques can be used with processes.
...
In SCL, \SCALA{Chan[A]}\SCLIndexX{chan}{Chan} is the type of channels that pass data
of type~\SCALA{A}.  (The type is polymorphic in~|A|: see
Section~\ref{sec:scala-types}.)  This type has subtypes
|SyncChan[A]|\SCLIndexX{syncChan}{SyncChan}, of synchronous channels, and
|BuffChanT[A]|\SCLIndexX{buffChan}{BuffChan}, of buffered (or asynchronous) channels.  We
will start by describing synchronous channels, and later describe buffered
channels.
...
Section~\ref{sec:scala-encapsulation}).  (The definition of |Chan[A]| uses
multiple inheritance: it inherits definitions from both |InPort[A]| and
|OutPort[A]|.)  The types \SCALA{InPort[A]}\SCLIndexX{inPort}{InPort} and
\SCALA{OutPort[A]}\SCLIndexX{outPort}{OutPort} can be abbreviated
as \SCALA{??[A]}\SCLIndex{??}
and~\SCALA{!![A]},\index[SCL]{\Ashriek\Ashriek} respectively.
%%% Note: the index command for ?? ensures the entry comes after that for ?.
