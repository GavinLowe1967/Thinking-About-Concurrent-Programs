...
to ensure threads do not interfere with one another.  In this book, we will
see four basic ways to achieve this: message passing, barrier
synchronisations, monitors and semaphores.  At a higher level of abstraction,
...
Think of a channel\index{channel} as a wire.  Messages are sent at one end,
...
In SCL, \SCALA{Chan[A]}\SCLIndexX{chan}{Chan} is the type of channels that
pass data of type~\SCALA{A}.  (The type is polymorphic in~|A|: see
Section~\ref{sec:scala-types}.)  This type has subtypes
|SyncChan[A]|\SCLIndexX{syncChan}{SyncChan}, of synchronous channels, and
|BuffChanT[A]|\SCLIndexX{buffChan }{BuffChan}, of buffered (or asynchronous)
channels.  We  start by describing synchronous channels, and later
describe buffered channels.

% Hack, because indexing uses "!" as a special character.  Note: the order of
% the index depends on the name of the macro!
\def\Ashriek{\SCALA{!}}
The command
\begin{scala}
  val chan = new SyncChan[A]
\end{scala}
defines |chan| to be a synchronous channel\index{channel!synchronous} passing
data of type~|A|.  Given such a channel, the
command\index[SCL]{\Ashriek}
\begin{scala}
  chan!v
\end{scala}
sends the value \SCALA{v} (of type~|A|) on \SCALA{chan}.  The
expression\SCLIndexX{? }{?} 
%%% Note: the index command for ? ensures the entry comes before that for ??.
\begin{scala}
  chan?()
\end{scala}
...
A channel is comprised of an \emph{in-port}\index{in-port} (something from
which threads can receive) and an \emph{out-port}\index{out-port} (something
on which threads can send).  Note that the terminology ``in-port'' and
``out-port'' refer to the point of view of \emph{threads}, not channels: a
thread receives values in on an in-port, and sends values out on an out-port;
however, a value is put into a channel on an out-port, and passed out on an
in-port.

Figure~\ref{fig:channel-types} outlines the relevant types (see
Section~\ref{sec:scala-encapsulation}).  An |InPort|\SCLIndexX{inPort}{InPort}
provides the operation~|?|, and an |OutPort|\SCLIndexX{outPort}{OutPort}
provides the operation~|!|.  The definition of |Chan[A]| uses multiple
inheritance: it inherits definitions from both |InPort[A]| and |OutPort[A]|,
so a |Chan| must provide implementations of both~|?| and~|!|.  The
types \SCALA{InPort[A]} and \SCALA{OutPort[A]} can be abbreviated
as \SCALA{??[A]}\SCLIndex{??}  and~\SCALA{!![A]},\index[SCL]{\Ashriek\Ashriek}
respectively.
%%% Note: the index command for ?? ensures the entry comes after that for ?.
...
known as \emph{fine-grained concurrency}\index{fine-grained concurrency}: the
system is built from several small components which are composed in parallel.
It is not a very efficient--- or even sensible---way to create real programs.
We include such examples here as they are a good way to get used to thinking
about concurrency---where multiple components work together---while keeping
the quantity of code small.  Such a program could be thought of as a model of
an electronic circuit, with components implemented in hardware and wired
together.
...
By contrast, buffered channels\index{channel!buffered} normally allow a
send to happen, even if there is no thread ready to receive: the sender can
return immediately, and the channel stores the values until a receiver is
ready to receive them.  In particular, the channel ensures the messages are
received in the same order in which they are sent: the channel acts as a
first-in first-out buffer.

There are various types of buffered channels in SCL, collected in the trait
|BuffChanT|:\SCLIndexX{buffChanT}{BuffChanT} 
\begin{scala}
trait BuffChanT[A] extends Chan[A]
\end{scala}
%
One class of buffered channels is defined as
follows.\SCLIndexX{unboundedBuffChan}{UnboundedBuffChan} 
%
\begin{scala}
...
%
The two types of bounded buffered channel are defined
as:\SCLIndexX{onePlaceBuffChan}{OnePlaceBuffChan}% 
\SCLIndexX{buffChan }{BuffChan}
%
\begin{scala}
