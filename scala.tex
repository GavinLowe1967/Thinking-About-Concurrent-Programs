...
The Scala operator |match| performs \emph{pattern matching}.  The command
%
\begin{scala}
  £$v$£ match{
    case £$pat_1$£ => £$cmd_1$£
...
\end{itemize}
%
Case classes (Section~\ref{sec:scala-encapsulation}), such as the |Option| type
(Section~\ref{sec:scala-types}), also allow for pattern matching.
%% \end{scalaBox}
...
An \emph{anonymous function} can be defined in Scala in a couple of ways.  The
notation \SCALA{(x: A) => exp} represents a function that takes an
argument~|x| of type~|A|, and returns the value of~|exp|.  For example,
\SCALA{(x: Int) => x+1} is a function that increments an |Int|.  The typing
``\SCALA{: A}'' can often be omitted, when the Scala compiler can deduce the
type; for example, in \SCALA{List(1,2,3).map(x => x+1)}, the compiler can
deduce that |x| must be an |Int|.
...
A \emph{\SCALA{for} expression} provides a convenient way of building sequences.  The
...
handler is normally written using pattern matching syntax (described above),
for example
\begin{scala}
  try{ prog } catch{
    case st: Stopped => ...
...
\framebox{TODO:} |import|,  equality and hash codes
(from datatypes3).
