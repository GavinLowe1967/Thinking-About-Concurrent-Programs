...
in the server.  Alternatively, we can use a single channel, and use different
subtypes of the channel's data type to represent the different sorts of
request.
...
flexible, so it is what we tend to use.  With either approach, the client-side
code is normally straightforward, differing little between examples.
...
% Notes: ``synchronisation objects'' paper; Ford-Fulkerson.

\section*{Exercises}

\input{Exercises/timedExchanger}

\input{Exercises/menWomenServer}

\input{Exercises/atomicBroadcast}

\input{Exercises/sameArgSync}
